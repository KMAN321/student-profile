                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?php echo ($result['gender'] == 0) ? 'M' : 'F'; ?></td>
                            <td><?php echo date('M d Y', strtotime($result['birthday'])); ?></td>
                    <?php } ?>
                    <?php
                    $results = $studentDetails->getAll();
                    foreach ($results as $result) {
                    ?>
                            <td><?php echo $results['contact_number']; ?></td>
                            <td><?php echo $results['street']; ?></td>
                            <td><?php echo $results['town_city']; ?></td>
                            <td><?php echo $results['province']; ?></td>
                            <td><?php echo $results['zip_code']; ?></td>
                    <?php } ?>
                            <td>
                                <a href="student_edit.php?id=<?php echo $result['id']; ?>" class="btn btn-primary">Edit</a>
                                <a href="student_delete.php?id=<?php echo $result['id']; ?>" class="btn btn-danger">Delete</a>
                            </td>
                        </tr>
                </tbody>
